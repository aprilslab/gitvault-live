...
- **Electron(Obsidian) 최소 PATH** — GUI 앱은 `/opt/homebrew/bin`·`/usr/local/bin` 빠진 PATH 로 실행 → 거기 설치된 `git-lfs`(및 훅 도구)를 못 찾아 LFS repo 의 checkout 훅·필터가 매번 실패한다. GitManager 가 git child PATH 를 보강(`enrichedGitPath`)해 해결. 터미널·launchd 데몬은 PATH 갖춰져 무영향 → **증상이 플러그인에서만 나고 데몬이 가려서 진단이 어려움**.
- **데몬↔플러그인 공존 = heartbeat lease + 히스테리시스.** 플러그인이 `.git/ogs-plugin-alive` 를 주기 갱신(정상 종료 시 삭제 → 즉시 인계). 데몬은 파일이 `TAKEOVER_GRACE_MS`(90s) 안이면 후퇴 — 일시 stale(플러그인이 자기 git op 로 바쁨)에 takeover 하면 HEAD 를 뺏어 wip 재fork → 경합·churn 이 났었다.
- **자동 파괴적 복구 금지.** 실패 시 자동 `reset --hard`/stash 복구를 돌리면, 복구의 워킹트리 쓰기가 vault 이벤트→재커밋→재경합 **피드백 루프**를 만든다(회귀 있었음). 복구는 수동(`recoverSession`/'기기 ID 재설정')만. 네트워크/오프라인 에러는 복구 대상 아님(`isNetworkError`) — '오프라인' 표시 후 다음 사이클 재시도.
