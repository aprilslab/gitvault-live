...
if (-not $Device) { $Device = ($env:COMPUTERNAME).ToLower() }
$RemoteEff = $Remote
...
# 환경변수 담은 실행 배치
$Cmd = Join-Path $Base "run-daemon.cmd"
@"
@echo off
...
"$NodeExe" "$Base\daemon.js" >> "$Base\daemon.log" 2>&1
"@ | Set-Content -Encoding ASCII $Cmd

# 부팅 시 시작 (로그인 계정)
schtasks /Create /TN "obsidian-git-sync" /SC ONSTART /RU "$env:USERNAME" /TR "$Cmd" /F | Out-Null
schtasks /Run /TN "obsidian-git-sync" | Out-Null
Write-Host "✓ daemon 상주 시작 (작업 스케줄러: obsidian-git-sync)"
Write-Host "  로그: Get-Content $Base\daemon.log -Wait   중지: schtasks /End /TN obsidian-git-sync; schtasks /Delete /TN obsidian-git-sync /F"
