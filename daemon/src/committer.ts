...
// takeover 히스테리시스: heartbeat 파일이 이 시간 안이면 데몬이 계속 후퇴한다. 플러그인이 자기 git op
// 로 바빠 갱신을 잠깐(수십초) 놓쳐도 takeover→churn 을 막는다(플러그인 갱신 주기 10s 의 9배).
// 플러그인은 정상 종료 시 파일을 삭제하므로 즉시 인계됨 — grace 는 크래시로 파일이 낡게 남은 경우에만 적용.
...
export class Committer {
  ...
  /**
   * 플러그인(Obsidian)이 이 vault 를 소유 중인가 — `.git/ogs-plugin-alive` 파일이 grace 안이면 true.
   * true 면 daemon 은 commit/merge 를 전면 후퇴한다(플러그인이 wip/저장 흐름 담당, 공유 .git 충돌 방지).
   *
   * [히스테리시스] 판정 기준은 30s 신선함이 아니라 TAKEOVER_GRACE_MS(90s) 다. 플러그인은 자기 git op
   * (fetch/merge/checkout) 로 바빠 heartbeat 갱신을 잠깐(수십초) 놓칠 수 있는데, 그 일시 stale 에 데몬이
   * takeover 하면 HEAD 를 main 으로 옮겨 플러그인이 즉시 wip 재fork → 경합·churn. grace 안에선 계속 후퇴한다.
   * 플러그인은 **정상 종료 시 이 파일을 삭제**하므로 그때는 즉시 인계된다 — grace 는 '크래시로 파일이 낡게
   * 남은' 경우에만 적용돼, 90s 뒤 데몬이 인수한다.
   */
  private pluginActive(): boolean {
    try {
      const ts = Number(readFileSync(join(this.cfg.vaultPath, '.git', HEARTBEAT_FILE), 'utf8').trim());
      return Number.isFinite(ts) && Date.now() - ts < TAKEOVER_GRACE_MS;
    } catch {
      return false; // 파일 없음(정상 종료/미실행) → 데몬 담당
    }
  }
