...
```bash
git clone <이 repo> && cd obsidian-git-sync
npm install
npm run build
# 산출물:
...
1. Obsidian 설정 → 커뮤니티 플러그인 → **제한 모드 해제** → obsidian-git-sync **활성화**.
2. 플러그인 설정 입력:
...
`~/Library/LaunchAgents/com.obsidian-git-sync.daemon.plist`:
...
<plist version="1.0"><dict>
  ...
  <key>ProgramArguments</key><array>
    ...
    <string>/opt/obsidian-git-sync/daemon.js</string>
  </array>
...
launchctl load ~/Library/LaunchAgents/com.obsidian-git-sync.daemon.plist
tail -f /tmp/ogs-daemon.log
```

### 3-3. Windows (작업 스케줄러)

1. 배치 파일 `C:\obsidian-git-sync\run-daemon.cmd`:
   ...
   node C:\obsidian-git-sync\daemon.js >> C:\obsidian-git-sync\daemon.log 2>&1
   ```

2. 등록 (관리자 PowerShell):

   ```powershell
   schtasks /Create /TN "obsidian-git-sync" /SC ONSTART /RU "%USERNAME%" `
     /TR "C:\obsidian-git-sync\run-daemon.cmd"
   schtasks /Run /TN "obsidian-git-sync"   # 즉시 시작
   ```

   로그인 세션과 무관하게 항상 돌리려면 [NSSM](https://nssm.cc) 으로 서비스 등록해도 된다:
   `nssm install obsidian-git-sync "C:\Program Files\nodejs\node.exe" C:\obsidian-git-sync\daemon.js`
   (환경변수는 `nssm set obsidian-git-sync AppEnvironmentExtra VAULT_PATH=... REMOTE=... DEVICE_ID=...`)
